...
export default function ArbitragePage() {
  ...
  useEffect(() => {
    ...
    const missing = top10.filter((symbol) => !isFreshAvg(avg30d[symbol]))
  ...
  }, [data?.topGainers, avg30d])

  const topGainersWithAvg = useMemo(() => (data?.topGainers ?? []).map((row, index) => {
    const stat = index < 10 ? avg30d[row.symbol] : undefined
  ...
  return (
    <main className="min-h-screen bg-slate-950 text-slate-100"><div className="mx-auto max-w-7xl px-3 py-4 sm:px-5 lg:px-6">
      <header className="mb-4 rounded-3xl border border-slate-800 bg-slate-900/80 p-4 shadow-2xl shadow-black/30"><div className="flex flex-col gap-3 lg:flex-row lg:items-end lg:justify-between"><div><p className="text-xs uppercase tracking-[0.25em] text-cyan-300">Vercel Arbitrage Monitor</p><h1 className="mt-1 text-2xl font-bold sm:text-3xl">Bitkub Gap Real-time</h1><p className="mt-1 text-sm text-slate-400">Top 24H ตัด broker coin ออก · Avg30D โหลดเฉพาะ Top 10 Gainer และจำไว้ 30 วัน</p></div><div className="flex flex-wrap items-center gap-2 text-sm"><span className="rounded-full border border-slate-700 bg-slate-950 px-3 py-1.5">USDT/THB: {data?.usdtThb ? fmt.format(data.usdtThb) : '-'}</span><span className="rounded-full border border-slate-700 bg-slate-950 px-3 py-1.5">Updated: {updatedAt}</span><span className="rounded-full border border-slate-700 bg-slate-950 px-3 py-1.5">Latency: {data?.latencyMs ?? '-'} ms</span><span className={`rounded-full border px-3 py-1.5 ${loading ? 'border-cyan-500 text-cyan-200' : 'border-slate-700 text-slate-300'}`}>{loading ? 'Scanning...' : auto ? 'Auto' : 'Manual'}</span></div></div></header>
