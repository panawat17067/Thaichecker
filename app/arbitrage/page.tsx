...
type ScanResponse = {
  ...
  rows: Row[]
  topGainers?: TopMarketRow[]
  topVolumes?: TopMarketRow[]
  brokerExcluded24h?: number
  logs: string[]
}
...
function modeButton(active: boolean, label: string, onClick: () => void) {
  return (
    <button
      onClick={onClick}
      className={`rounded-xl border px-4 py-2 text-sm font-semibold ${active ? 'border-cyan-300 bg-cyan-400 text-slate-950' : 'border-slate-700 bg-slate-950 text-slate-300 hover:border-cyan-400'}`}
    >
      {label}
    </button>
  )
...
export default function ArbitragePage() {
  ...
  const marketRows = useMemo(() => {
    const q = query.trim().toUpperCase()
    const source = viewMode === 'gainers' ? data?.topGainers : data?.topVolumes
  ...
  return (
    <main className="min-h-screen bg-slate-950 text-slate-100">
      <div className="mx-auto max-w-7xl px-3 py-4 sm:px-5 lg:px-6">
        ...
        <section className="mb-4 flex flex-wrap gap-2 rounded-3xl border border-slate-800 bg-slate-900/70 p-3">
          {modeButton(viewMode === 'arbitrage', 'Arbitrage', () => setViewMode('arbitrage'))}
          {modeButton(viewMode === 'gainers', 'Top 24H Gainer', () => setViewMode('gainers'))}
          {modeButton(viewMode === 'volumes', 'Top 24H Volume', () => setViewMode('volumes'))}
          <span className="rounded-xl border border-slate-800 bg-slate-950 px-3 py-2 text-xs text-slate-400">24H excludes broker: {data?.brokerExcluded24h ?? 0}</span>
        </section>
        ...
        <section className="mb-3 grid grid-cols-2 gap-3 md:grid-cols-4">
          ...
          <div className="rounded-2xl border border-slate-800 bg-slate-900 p-4"><p className="text-xs text-slate-400">Top Gainer</p><p className="text-lg font-bold text-emerald-300">{topGainer ? `${topGainer.symbol} ${pctFmt.format(topGainer.percentChange)}%` : '-'}</p></div>
          <div className="rounded-2xl border border-slate-800 bg-slate-900 p-4"><p className="text-xs text-slate-400">Top 24H Volume</p><p className="text-lg font-bold">{topVolume ? `${topVolume.symbol} ${volFmt.format(topVolume.quoteVolume)}` : '-'}</p></div>
            ...
            <div className="overflow-x-auto">
              <table className="min-w-[1120px] w-full text-left text-sm">
                ...
                <tbody className="divide-y divide-slate-800">
                  ...
                  {rows.length === 0 && <tr><td colSpan={9} className="px-4 py-10 text-center text-slate-500">ยังไม่มีข้อมูล หรือ exchange ปลายทางตอบช้า ลองกด Scan อีกครั้ง</td></tr>}
                </tbody>
              </table>
            </div>
          </section>
              ...
              <table className="min-w-[920px] w-full text-left text-sm">
                ...
                <tbody className="divide-y divide-slate-800">
                  {marketRows.map((row, index) => (
                    <tr key={`${viewMode}-${row.symbol}`} className="hover:bg-slate-800/60">
                  ...
                  {marketRows.length === 0 && <tr><td colSpan={7} className="px-4 py-10 text-center text-slate-500">ยังไม่มีข้อมูล Top 24H หรือ market/symbols ยังไม่ตอบ</td></tr>}
                </tbody>
              </table>
            </div>
          </section>
        ...
        <section className="mt-4 rounded-3xl border border-slate-800 bg-slate-900/70">
          ...
          {showLogs && <pre className="max-h-80 overflow-auto border-t border-slate-800 p-4 text-xs text-slate-400">{logs.length ? logs.join('\n') : 'No logs'}</pre>}
        </section>
      </div>
    </main>
  )
